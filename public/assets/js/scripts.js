...
!(function (NioApp, $) {
    ...
    NioApp.Custom.Stepper = function (elm, opt) {
        // data
        var data_step_init =
            elm.dataset.stepInit && !isNaN(elm.dataset.stepInit)
                ? parseInt(elm.dataset.stepInit)
                : false;
        var settings = {
            selectors: {
                nav: opt.selectors.nav ? opt.selectors.nav : "stepper-nav",
                progress: opt.selectors.progress
                    ? opt.selectors.progress
                    : "stepper-progress",
                content: opt.selectors.content
                    ? opt.selectors.content
                    : "stepper-steps",
                prev: opt.selectors.prev ? opt.selectors.prev : "step-prev",
                next: opt.selectors.next ? opt.selectors.next : "step-next",
                submit: opt.selectors.submit
                    ? opt.selectors.submit
                    : "step-submit",
            },
            classes: {
                nav_current: opt.classes.nav_current
                    ? opt.classes.nav_current
                    : "current",
                nav_done: opt.classes.nav_done ? opt.classes.nav_done : "done",
                step_active: opt.classes.step_active
                    ? opt.classes.step_active
                    : "active",
                step_done: opt.classes.step_done
                    ? opt.classes.step_done
                    : "done",
            },
            current_step: data_step_init
                ? data_step_init
                : parseInt(opt.current_step),
        };
        var nav_items = elm.querySelectorAll(
                ".".concat(settings.selectors.nav, " > *")
            ),
            step_items = elm.querySelectorAll(
                ".".concat(settings.selectors.content, " > *")
            ),
            step_progress = elm.querySelector(
                ".".concat(settings.selectors.progress)
            ),
            step_progress_count = elm.querySelector(
                ".".concat(settings.selectors.progress, "-count")
            ),
            step_progress_bar = elm.querySelector(
                ".".concat(settings.selectors.progress, "-bar")
            ),
            step_prev = elm.querySelector(".".concat(settings.selectors.prev)),
            step_next = elm.querySelector(".".concat(settings.selectors.next)),
            step_submit = elm.querySelector(
                ".".concat(settings.selectors.submit)
            ),
            ...
            _step_nav =
                elm.querySelectorAll(".".concat(settings.selectors.nav))
                    .length > 0
                    ? true
                    : false,
            _step_progress =
                elm.querySelectorAll(".".concat(settings.selectors.progress))
                    .length > 0
                    ? true
                    : false;
        //Show step function
        function showStep(_current) {
            ...
            if (_step_progress) {
                step_progress_count.innerHTML = ""
                    .concat(_current, " of ")
                    .concat(_step_count);
                step_progress_bar.style.width = "".concat(
                    (100 / _step_count) * _current,
                    "%"
                );
            }
        }
